...
eLabel* Layout::addeLabel(const sf::String& string, int id)
{

	eLabel* elabel = new eLabel(string);
	add(elabel, id);
	return elabel;
}
...
void Layout::depush(int id)
{
	for (Widget* widget = getFirstWidget(); widget != NULL; widget = widget->m_next)
	{
		if (widget->getID() == id)
		{
			if (widget == m_first&&widget == m_last) {
				m_last = NULL;
				m_first = NULL;
			}
			else if (m_first == widget)
			{
				m_first = widget->m_next;
				widget->m_next->m_previous = NULL;
			}
			else if (m_last == widget)
			{
				m_last = widget->m_previous;
				widget->m_previous->m_next = NULL;
			}
			else 
			{
				widget->m_previous->m_next = widget->m_next;
				widget->m_next->m_previous = widget->m_previous;
			}
			delete widget;
			break;
...
void Layout::onKeyPressed(sf::Keyboard::Key key)
{
	...
	else if (key == Theme::nextWidgetKey2)
	{
		if (!focusNextWidget())
			// Try to focus first widget if possible
			focusNextWidget();
