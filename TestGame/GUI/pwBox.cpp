...
pwBox::pwBox(float width):
	...
	void pwBox::setText(const sf::String& string)
	{
	...
	const sf::String& pwBox::getText() const
	{
		return m_text.getString();
	}

	const sf::String& pwBox::getpwText() const
	{
		return m_pwtext.getString();
	}

	void pwBox::setCursor(size_t index)
	{
	...
	size_t pwBox::getCursor() const
	{
		return m_cursor_pos;
	}


	void pwBox::onKeyPressed(sf::Keyboard::Key key)
	{
		switch (key)
		{
		...
		case sf::Keyboard::BackSpace:
			...
			if (m_cursor_pos > 0)
			{
				...
				sf::String pwstring = m_pwtext.getString();
				string.erase(m_cursor_pos - 1);
				pwstring.erase(m_cursor_pos - 1);
				m_text.setString(string);
				m_pwtext.setString(pwstring);
	...
	void pwBox::onMousePressed(float x, float y)
	{
	...
	void pwBox::onTextEntered(sf::Uint32 unicode)
	{
		if (unicode > 30 && (unicode < 127 || unicode > 159))
		{
			...
			if (m_max_length == -1 || (int)string.getSize() < m_max_length)
			{
				...
				pwstring.insert(m_cursor_pos, unicode);
	...
	void pwBox::onStateChanged(State state)
	{
		m_box.applyState(state);
	}


	void pwBox::draw(sf::RenderTarget& target, sf::RenderStates states) const
	{
